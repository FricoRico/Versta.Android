...
description: The Subtle Gradient Design System — a financial-infrastructure design language built on a deep navy ink, an electric indigo primary, and a recurring atmospheric gradient mesh that occupies the upper third of nearly every marketing page. The system pairs the open-source Inter family at thin (300) weights with negative letter-spacing for editorial-density display headlines, and uses tabular-figure body type where money and numerics matter. Buttons are tight-radius pills, cards live on near-white surfaces, and the dashboard track flips polarity to a familiar dark-app shell.
...
typography:
  display-xxl:
    fontFamily: "inter-var, 'SF Pro Display', system-ui, -apple-system, sans-serif"
    fontSize: 56px
    fontWeight: 300
    lineHeight: 1.03
    letterSpacing: -1.4px
...
> **Source pages:** marketing home, product feature pages, pricing, and dashboard onboarding surfaces.
...
- **Purple Bridge** (`#8d47f7`): Gradient middle stop between indigo and magenta in the app runtime mesh. Lavender (`#b9b9f9`) remains for pill/tag fills; it is retired from gradient use.
...
The display and UI tier is **Inter** (open-source, SIL Open Font License) at weights 300 (thin) and 400 (regular). The variable font (`inter-var`) is loaded with `font-feature-settings: "ss01"` enabled globally — the stylistic set substitutes character variants that are part of the system's typographic signature.
...
**Inter** is open-source (available via Google Fonts and self-hostable as the `inter-var` variable font) across all required weights. Use weight 300 with `letter-spacing: -1.4px` and `font-feature-settings: "ss01"` for display tiers, and 300 with `font-feature-settings: "tnum"` where applicable for body sizes. Avoid Helvetica or heavier system defaults — they're heavier than the system needs.
...
## Gradient mesh in the Android app

The app-wide backdrop is a bottom-edge gradient wash: color is strongest in the bottom corners and fades upward into the system color. The upper screen always stays on the pure system color. The wash forms a gentle U: the corners (indigo `#533afd` left, magenta `#f96bee` right) reach slightly higher than the purple bridge (`#8d47f7`) in the middle.

In light mode the accents are washed toward white; in dark mode the same tints are turned deep and saturated rather than pale. Exact derivations are computed in code; this documents intent, not constants.
...
- Don't move the in-app mesh to the top of the screen or tint the upper screen; in the Android app the backdrop stays bottom-anchored.
- Don't reintroduce lavender (`#b9b9f9`) as a gradient stop; the middle stop is the purple bridge (`#8d47f7`).
